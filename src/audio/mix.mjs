...
function meanVolume(file, startSeconds, durationSeconds) {
  const result = run(
    'ffmpeg',
    ['-hide_banner', '-ss', String(startSeconds), '-t', String(durationSeconds), '-i', file, '-vn', '-af', 'volumedetect', '-f', 'null', '-'],
    'MUSIC_WINDOW_ANALYSIS_FAILED'
  );
  const match = (result.stderr || '').match(/mean_volume:\s*(-?[0-9.]+) dB/);
  return match ? Number(match[1]) : -99;
}

function findAudibleWindows(file) {
  const probe = JSON.parse(run('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'json', file], 'MUSIC_PROBE_FAILED').stdout);
  const duration = Number(probe.format?.duration ?? 0);
  invariant(duration > 1, 'MUSIC_DURATION_INVALID', `Music asset has invalid duration: ${file}`);

  const windowSeconds = Math.min(6, Math.max(2, duration / 8));
  const stepSeconds = Math.max(2, Math.min(8, windowSeconds));
  const samples = [];

  for (let start = 0; start <= Math.max(0, duration - windowSeconds); start += stepSeconds) {
    samples.push({start, mean_volume_db: meanVolume(file, start, windowSeconds)});
  }

  samples.sort((a, b) => b.mean_volume_db - a.mean_volume_db || a.start - b.start);
  const audible = samples.filter((sample) => sample.mean_volume_db > -38);
  invariant(audible.length > 0, 'MUSIC_ASSET_EFFECTIVELY_SILENT', `No audible source window found in ${file}`);

  return audible.slice(0, Math.min(8, audible.length)).map((sample) => sample.start);
}

function buildMusicFilters(plan, timeline, inputIndexByPath, audibleWindowsByPath) {
  const filters = [];
  const labels = [];
  ...
  for (const [index, cue] of plan.music_cues.entries()) {
    ...
    const sourceOffset = windows[index % windows.length];
  ...
  filters.push(`${labels.join('')}amix=inputs=${labels.length}:duration=longest:normalize=0,atrim=duration=${timeline.transformed_duration_seconds},asetpts=PTS-STARTPTS[music_base]`);

  const pauseExpression = timeline.editorial_pauses.length
    ? timeline.editorial_pauses.map((pause) => `between(t,${pause.output_start_seconds},${pause.output_end_seconds})`).join('+')
    : '0';
  const representativeCueDb = plan.music_cues.reduce((sum, cue) => sum + cue.gain_db, 0) / plan.music_cues.length;
  const relativeFloorDb = Math.min(0, plan.music_floor.narration_floor_db - representativeCueDb);

  filters.push('[music_base]asplit=2[music_for_duck][music_floor_src]');
  filters.push(`[music_floor_src]volume=${dbToLinear(relativeFloorDb)},volume='if(gt(${pauseExpression},0),${dbToLinear(plan.music_floor.pause_boost_db)},1)':eval=frame[music_floor]`);
  return filters;
}
...
export async function buildAudioMix({projectId, audioPlan, timeline}) {
  ...
  const audibleWindowsByPath = new Map();
  ...
  for (const [index, relative] of audioPlan.music_assets.entries()) {
    ...
    audibleWindowsByPath.set(relative, findAudibleWindows(musicPaths[index]));
  }
  ...
  const codec = path.extname(output).toLowerCase() === '.wav'
    ? ['-c:a', 'pcm_s24le']
  ...
  const metadata = {
    ...
    audible_windows: Object.fromEntries(audibleWindowsByPath),
