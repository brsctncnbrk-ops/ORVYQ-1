...
export function validateNarrationTimeline(timeline) {
  ...
  for (const [index, word] of words.entries()) {
    ...
    number(word.output_start, 'TIMELINE_WORD_OUTPUT_START_INVALID', `Timeline word ${index} output_start is invalid`, {min: lastWordOutputEnd});
    ...
    lastWordOutputEnd = word.output_end;
  }
  ...
  for (const pause of pauses) {
    ...
    number(pause.output_start_seconds, 'PAUSE_START_INVALID', `${pause.pause_id} output_start_seconds is invalid`, {min: previousPauseOutputEnd});
    ...
    previousPauseOutputEnd = declaredEnd;
  ...
  invariant(lastWordOutputEnd <= timeline.transformed_duration_seconds, 'TIMELINE_WORDS_EXCEED_DURATION', 'Aligned words exceed transformed duration');
  invariant(timeline.transformed_duration_seconds - lastWordOutputEnd <= 15, 'TIMELINE_FINAL_RELEASE_TOO_LONG', 'Final release after the last word cannot exceed 15 seconds');
  return timeline;
}
