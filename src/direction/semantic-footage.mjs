...
export function selectSemanticFootage(productionPlan, {claimRegistry, assetRegistry}) {
  ...
  const shots = productionPlan.shots.map((shot) => {
    ...
    const ranked = candidates.map((asset) => ({asset, ...scoreAsset({asset, intent, recentAssetIds: recentAssetIds.slice(-2), globalUse})}))
      .sort((a, b) => b.score - a.score || b.matches - a.matches || a.asset.asset_id.localeCompare(b.asset.asset_id));
  ...
  return {productionPlan: {...productionPlan, shots}, report: {schema_version: '1.0', status: 'TAMAMLANDI', direct_selection_count: report.filter((item) => item.mode === 'direct_semantic').length, fallback_selection_count: report.filter((item) => item.mode !== 'direct_semantic').length, selections: report}};
}
