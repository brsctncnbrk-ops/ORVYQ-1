...
const sourceCatalog = validateSourceCatalog({schema_version: '1.0', project_id: 'test-film', sources: [{source_id: 'SRC_OFFICIAL', title: 'Official report', publisher: 'Official Publisher', url: 'https://example.gov/report.pdf', kind: 'primary', active: true, required: true, accessed_at: '2026-07-21T00:00:00.000Z', license_or_basis: 'Official publication used for criticism and commentary'}]});
const claimRegistry = validateClaimRegistry({schema_version: '1.0', project_id: 'test-film', claims: [{claim_id: 'CLM_ONE', text: 'The official report describes a documented threshold.', section_id: 'SEC_ONE', status: 'verified', source_ids: ['SRC_OFFICIAL'], visual_evidence_requirement: 'required', confidence: 0.95}]}, {sourceCatalog});

test('primary evidence rejects SVG editorial cards', () => {
  assert.throws(() => validateEvidenceRegistry({schema_version: '1.0', project_id: 'test-film', evidence: [{evidence_id: 'EVID_ONE', evidence_type: 'primary_document', relative_path: 'evidence/documents/card.svg', sha256: 'a'.repeat(64), byte_size: 5000, source_url: 'https://example.gov/report.pdf', source_ids: ['SRC_OFFICIAL'], claim_ids: ['CLM_ONE'], provenance_mode: 'official_primary_capture', caption: 'Official report page', license_or_basis: 'Official publication'}]}, {sourceCatalog, claimRegistry}), (error) => error instanceof OrvyqError && error.code === 'PRIMARY_SOURCE_PIXELS_REQUIRED');
});

test('primary evidence accepts real raster capture metadata', () => {
  const registry = validateEvidenceRegistry({schema_version: '1.0', project_id: 'test-film', evidence: [{evidence_id: 'EVID_ONE', evidence_type: 'primary_document', relative_path: 'evidence/documents/page.png', sha256: 'a'.repeat(64), byte_size: 5000, source_url: 'https://example.gov/report.pdf', source_ids: ['SRC_OFFICIAL'], claim_ids: ['CLM_ONE'], provenance_mode: 'official_primary_capture', caption: 'Official report page one', license_or_basis: 'Official publication', page_number: 1}]}, {sourceCatalog, claimRegistry});
...
test('audio plan requires four distinct states and continuous coverage', () => {
  const base = {
    ...
    ducking: {enabled: true, threshold_db: -24, ratio: 3, attack_ms: 30, release_ms: 450},
    music_floor: {narration_floor_db: -24, pause_floor_db: -18, pause_boost_db: 6},
...
test('production plan rejects metric-driven half-second relief cuts', () => {
  const plan = {schema_version: '1.0', project_id: 'test-film', fps: 30, width: 1920, height: 1080, full_frame_count: 18000, full_duration_seconds: 600, proof_boundary_frame: 4500, audio_mix_asset: 'assets/audio/final_mix.mp3', sections: [{section_id: 'A', title: 'A', start_frame: 0, end_frame: 9000}, {section_id: 'B', title: 'B', start_frame: 9000, end_frame: 18000}], shots: [{shot_id: 's1', shot_type: 'contextual_footage', start_frame: 0, end_frame: 15, claim_ids: ['CLM_ONE'], asset_ids: [], editorial_purpose: 'Provide meaningful contextual visual support', motif_id: 'm1', evidence_claim: false}, {shot_id: 's2', shot_type: 'brand_close', start_frame: 15, end_frame: 18000, claim_ids: [], editorial_purpose: 'Close the complete documentary with the ORVYQ identity', motif_id: 'm2'}]};
