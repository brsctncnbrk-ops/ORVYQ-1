...
export function assertValidProjectId(projectId) {
  if (typeof projectId !== "string" || !PROJECT_ID_PATTERN.test(projectId)) {
    throw new CliError(`Invalid project id "${projectId ?? ""}". Expected a value like 001-my-video.`, "INVALID_PROJECT_ID");
  }
  const resolved = path.resolve(PROJECTS_DIR, projectId);
  const projectsRoot = path.resolve(PROJECTS_DIR) + path.sep;
  if (!resolved.startsWith(projectsRoot)) throw new CliError("Project path escapes the projects directory.", "INVALID_PROJECT_ID");
  return projectId;
}
