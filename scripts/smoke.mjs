...
run('ffmpeg', ['-y', '-f', 'lavfi', '-i', 'testsrc2=size=1920x1080:rate=30:duration=12', '-vf', 'eq=brightness=-0.38:saturation=0.75', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', path.join(project, 'assets', 'footage', 'smoke.mp4')]);
...
const renderInput = {
  ...
  plan: {
    ...
    shots: [
      {shot_id: 'hook', shot_type: 'cinematic_hook', start_frame: 0, end_frame: 120, claim_ids: [], asset_ids: ['SMOKE_VIDEO'], editorial_purpose: 'Verify dark cinematic footage receives controlled shadow lift without losing detail', motif_id: 'smoke-hook', motion: 'push', title: 'A SYSTEM UNDER PRESSURE', source_label: 'ORVYQ · DARK FOOTAGE FIXTURE'},
      {shot_id: 'pause', shot_type: 'editorial_pause', start_frame: 120, end_frame: 240, claim_ids: [], editorial_purpose: 'Verify an editorial pause keeps an opaque visible background throughout the transition', motif_id: 'smoke-pause', title: 'NO BLACK TRANSITION', subtitle: 'The background remains visible while the message resolves.', captions_suppressed: true, transition_in: 'short_dissolve', transition_out: 'short_dissolve'},
      {shot_id: 'document', shot_type: 'primary_document', start_frame: 240, end_frame: 330, claim_ids: [], evidence_ids: ['SMOKE_EVIDENCE'], editorial_purpose: 'Verify real raster evidence rendering at readable scale', motif_id: 'smoke-document', title: 'OFFICIAL SOURCE PIXELS', subtitle: 'The renderer displays a raster capture instead of a generated evidence card.', source_label: 'SMOKE · PRIMARY DOCUMENT'},
      {shot_id: 'close', shot_type: 'brand_close', start_frame: 330, end_frame: 360, claim_ids: [], editorial_purpose: 'Verify clean ORVYQ brand closing composition', motif_id: 'smoke-close', title: 'BEYOND THE KNOWN', subtitle: 'Canonical renderer smoke test'}
    ]
  },
  timeline: {editorial_pauses: [{pause_id: 'smoke-pause', output_start_seconds: 4, output_end_seconds: 8, duration_seconds: 4}], words: [{text: 'known', output_start: 10.5, output_end: 11.5}]},
...
const qa = renderedMediaQa(output, {expectedDuration: 12, editorialPauses: renderInput.timeline.editorial_pauses, shotBoundarySeconds: [4, 8, 11]});
